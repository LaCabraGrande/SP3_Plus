...
public class MainMenu extends AMenu {
    ...
    public void display() {
        ...
        while (choosingAction) {
            textUI.displayMessage("""
                    ...
                    3. Search by Rating
                    4. See list of watched Series And Movies
                    5. See list of favorites
                    6. Log out
                    """);
    ...
    private void searchByRating() {
        boolean searching = true;
        while (searching) {
            String rating = textUI.getInput("\nSearch by Rating: ");
            ArrayList<AMedia> mediasContainingInput = searchMediaByRating(rating);
            if (mediasContainingInput.isEmpty()) {
                textUI.displayMessage("\n*** No Movies or Series with this rating ***\n");
            } else {
                String ratingString = "Movies and Series with at least "+rating+" in rating";
                String searchResult = showSearchResults(mediasContainingInput, ratingString);
                if (searchResult.equalsIgnoreCase(exit)) {
                    searching = false;
                }
            }
